...
void delete_d_array (dynamic_array &d){
	if (d.size > 0){
		delete[] d.store;
		d.size = 0; // Di conseguenza non ho necessità di scrivere d.store = nullptr, in quanto il puntatore d.store non punterà a nulla in quanto size = 0; Tutto dipende dalle funzioni che uso, se le ho create io so rintracciare gli eventuali problemi
	}
	else{
...
int main()
{
	...
	catch (string& s) { // Gestisce tutte le eccezioni sollevate di tipo stringa (relative alle eccezioni sollevate da questo try)
		cerr << s << endl;
	}
