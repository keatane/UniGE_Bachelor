...
int pesoTotaleArchi(const Grafo& g) {
   int peso = 0;
   for(int i=0; i<maxNumVertici; ++i)
         for(int j=0; j<maxNumVertici; ++j)
            if (g.matrice[i][j] > 0) peso += g.matrice[i][j];
    return peso;
...
bool sonoCollegatiDaUnVertice(const Grafo& g, VerticeID v1, VerticeID v2, VerticeID& vlink) {
    ...
    for(int i=0; i<maxNumVertici; ++i)
        if(g.matrice[v1][i] > 0 && g.matrice[i][v2] > 0){ // se v1 è in relazione (basta > 0 ) con i e i è in relazione con v2, entrambi in modo diretto, allora i congiunge v1 e v2.
            vlink = i;
            return true;
        }
    return false;
