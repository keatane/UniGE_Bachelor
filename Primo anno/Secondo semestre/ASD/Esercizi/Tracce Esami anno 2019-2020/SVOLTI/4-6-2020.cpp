...
void stampaAux(const Tree& t) {
    if(!t) return;
    if(t->sinistro)
        cout << "S " << t->label << " " << t->sinistro->label << " \n";
    if(t->destro)
        cout << "D " << t->label << " " << t->destro->label << " \n";
...
void contaAux(const Tree& t, int& counter) {
    if(!t) return;
    Tree aux;
    if(vuoto(t->sinistro) ^ vuoto(t->destro)){ // verifico che vi sia un nodo figlio senza fratelli
        aux = (vuoto(t->sinistro)) ? t->destro : t->sinistro; // prendo il figlio non vuoto
        if(vuoto(aux->sinistro) && vuoto(aux->destro)) counter++; // se questo non ha figli sono giunto a una foglia
    }
    contaAux(t->sinistro, counter);
    contaAux(t->destro, counter);
}
...
int main() {
	...
	inserisciFiglioSX(1, 2, t);
    inserisciFiglioSX(2, -4, t);
    inserisciFiglioSX(-4, 7, t);
    inserisciFiglioSX(7, 8, t);
    */
