...
void print_list(std::ostream& output_stream, basic_list list) {
   // *** INSERIRE QUI IL VOSTRO CODICE, E' POSSIBILE AGGIUNGERE FUNZIONI AUSILIARIE SE NECESSARIO *** 
   if(list == emptyList) return;
   basic_list aux = list;
   while(aux->next != list){
        output_stream << aux->payload.family_name << " " << aux->payload.first_name << ", nato nel " << aux->payload.born_on << " a " << aux->payload.born_in << endl;
        aux = aux->next;
   }
    output_stream << aux->payload.family_name << " " << aux->payload.first_name << ", nato nel " << aux->payload.born_on << " a " << aux->payload.born_in << endl;
}
