...
namespace stack {
   ...
   void make_empty(Stack&);         /* Svuota la pila (cioe` cancella tutti gli elementi presenti */
}
...
Stack stack_intersection(Stack& s1, Stack& s2) {
    Stack s3 = EMPTYSTACK;
    if(isEmpty(s1) || isEmpty(s2)) return s3;
    Elem e1 = pop(s1);
    Elem e2 = pop(s2);
    
    while(e1 != EMPTYELEM && e2 != EMPTYELEM){ // appena uno dei due è vuoto, gli elem rimanenti non saranno comuni
        if(e1 < e2)
            e1 = pop(s1);
        else if(e1 > e2)
            e2 = pop(s2);
            ...
            push(e1, s3);
            e1 = pop(s1); // aggiorno entrambi i valori
            e2 = pop(s2);
        }
    }
    
    return s3; // ISTRUZIONE AGGIUNTA AI SOLI FINI DELLA COMPILAZIONE
}
