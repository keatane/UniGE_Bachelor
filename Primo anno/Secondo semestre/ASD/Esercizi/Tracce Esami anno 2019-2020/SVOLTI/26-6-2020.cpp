...
int intersection(const int a[], const int b[], int c[]) {
    int k = 0; // indice ausiliario, che farà anche da size di c
    for(int i = 0; i<dim; ++i)
        for(int j = 0; j<dim; ++j)
            if(a[i] == b[j]){
                c[k] = a[i];
...
bool equal(const int a[], const int b[]) {
    // controllo sulle size non necessario: entrambi sono di dim dimensione
    int c[dim] = {-1000000};
    intersection(a,b,c); // se l'intersezione fra a e b restituisce interamente a allora tutti gli elementi di a sono comuni a b
    if(c[0] == -1000000) return false; // intersezione vuota
