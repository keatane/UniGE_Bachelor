...
Stack stack_union(Stack& s1, Stack& s2) {
    ...
    if(isEmpty(s1)) return s_union = s2;
    if(isEmpty(s2)) return s_union = s1;
    
    while(!isEmpty(s1) && !isEmpty(s2)){
        if(s1->elem < s2->elem){
            push(s1->elem, s_union);
            s1 = s1->next;
        }
        else if (s1->elem > s2->elem){
            push(s2->elem, s_union);
            s2 = s2->next;
        }
        else{ // perchè uguali, porto avanti le due liste
            push(s1->elem, s_union);
            s1 = s1->next;
            s2 = s2->next;
        }
...
Stack stack_union(Stack& s1, Stack& s2) {
    ...
    while(e1 != EMPTYELEM && e2 != EMPTYELEM){
        if(e1 < e2){
            push(e1, s_union);
            e1 = pop(s1);
        }
