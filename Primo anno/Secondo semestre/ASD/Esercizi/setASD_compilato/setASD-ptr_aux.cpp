...
// Implementarla per esercizio
	...
	cell* s1_aux=s1;
	while (s1_aux != emptySet){
		if (!member(s1_aux->elem, s2))
			insertElemInOrderS(s1_aux->elem, s3); // risulteranno gli elementi di s1 non comuni a s2
		s1_aux = s1_aux->next;
	}
...
// s1 sottoinsieme di s2
bool set::isSubset(const Set& s1, const Set& s2)
{
// Implementarla per esercizio

	// Se dimensione del sottoinsieme > insieme, non è sottoinsieme
	if(size(s1) > size(s2))
		return false;
	cell* s1_aux=s1;
	while (s1_aux != emptySet){
	// Se almeno un elemento del sottoinsieme non è nell'insieme allora non è sottoinsieme
		if (!member(s1_aux->elem, s2))
			return false;
		s1_aux = s1_aux->next;
	}
