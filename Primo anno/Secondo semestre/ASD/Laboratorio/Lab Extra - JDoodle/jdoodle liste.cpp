...
void list::copyIfLowerThan(const list::List& l, list::List& l1, const int th) {
    for(List aux = l->next; aux != l; aux=aux->next) // mi fermo quando aux punta alla sentinella
        if(aux->info < th) addRear(aux->info, l1);
} 
...
int list::sum(const list::List& l) {
    int sum = 0;
    for(List aux = l->next; aux != l; aux=aux->next)
        sum += aux->info;
    return sum;
}
