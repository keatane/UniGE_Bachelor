...
/*** TO BE DONE START ***/
    ...
    // Azzeramento dei parametri di dimensione - non necessario de-allocando la struttura my_v successivamente
    // my_v->no_e = 0; 
    // my_v->cur_cap = 0;
...
/*** TO BE DONE START ***/
	...
	// Mi muovo dal puntatore a i (tenendo conto della quantità di byte x elemento)
...
/*** TO BE DONE START ***/
	...
	// Ridimensiono la sequenza
	v->e_array = check_a(realloc(v->e_array, (v->cur_cap)*v->e_sz));
	
	// e_sz = dimensione del tipo
	// devo tener conto della quantità di elementi (cur_cap) e ognuno di una quantità di byte da riservare data dal tipo
...
/*** TO BE DONE START ***/
	...
	// printf("%c",v[vector_size(v)-1]); // Stampa non necessaria
	
	// Precedentemente ho shiftato a destra gli elementi, l'elemento rimasto "fuori" viene rimosso
	// tramite un ridimensionamento 
	// Ridimensiono la sequenza (elimino l'ultimo elemento)
	v->e_array = check_a(realloc(v->e_array, v->cur_cap*v->e_sz));
...
// Inserisco n volte un elemento ad un certo indice
...
// Shift degli elementi a destra tramite una copia nella destinazione
...
// Inserimento dei nuovi valori (copia degli elementi)
...
// Rimuovo gli elementi rimanenti
