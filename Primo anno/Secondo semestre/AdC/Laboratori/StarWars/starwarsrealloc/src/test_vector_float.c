...
int main(){
    ...
    for ( i1 = 0 ; i1 < 10 ; i1++ ){
    	// Similmente a quanto fatto sotto con il pi-greco, scelgo qui il numero di Nepero per riempire il vector di float
    	float aux = i1 * 2.718281828; /*** approximation of Nepero's number ***/
        v_push_back(vi1,(void*)&aux);
        }

    printf("Vector vi1 now contains %d integers: ",vector_size(vi1));
    for ( i1 = 0 ; i1 < vector_size(vi1) ; i1++ )
        printf(" %f",vector_float_at(vi1,i1));
    printf("\n");

    for ( i1 = 0 ; i1 < vector_size(vi1) ; i1++ ) {
        c1 = '0' + i1; //vector_float_at(vi1,i1); /*** converting decimal number to char ***/
